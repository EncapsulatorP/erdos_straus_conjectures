...
Every positive rational has a finite expansion, so $L_{\mathrm{UF}}$ is always defined and finite. The interesting object is the bounded-length question $L_{\mathrm{UF}}(x) \le 3$.

For completeness:

$$
\mathcal{E}_* = \bigcup_{t \ge 1} \mathcal{E}_t
$$
...
| Schinzel (general) | $L_{\mathrm{UF}}(k/n) \le 3$ for all sufficiently large $n$ (given $k$) | open |
...
$$
\frac{k}{n} = \underbrace{\frac{1}{n} + \frac{1}{n} + \cdots + \frac{1}{n}}_{k\text{ copies}} \in \mathcal{E}_k \subseteq \mathcal{E}_t \qquad (k \le t)
$$

So at $t=3$, $1/n$, $2/n$, and $3/n$ are immediate, while $4/n$ is the first overflow case.

Pattern:
- $t=2$: threshold $k=3$ fails (for example, $3/7$ has no two-term unit-fraction form).
...
$$
\frac{k}{n} = \frac{1}{a} + \frac{1}{b} + \frac{1}{c}
$$
...
Fix the first denominator as $a = nt$. Then

$$
\frac{4}{n} - \frac{1}{nt} = \frac{4t - 1}{nt} = \frac{1}{b} + \frac{1}{c}
$$
...
Hence solutions correspond to divisor pairs $r, s \mid n^2 t^2$ with $rs = n^2 t^2$ such that
...
Applying ESC directly at even $N = p + q$ is weak because parity gives trivial closures. The more informative test is on prime fibers: compute an inversion obstruction score on $p, q$ (or shifted forms), then compare it against additive predictors.
...
$$
\text{ESC: } L_{\mathrm{UF}}(4/n) \le 3 \quad \text{for all } n \ge 2
$$

$$
\text{General direction: } L_{\mathrm{UF}}(k/n) \le 3
$$
...
2. Computes Erdős–Straus inversion statistics over the prime fiber.
3. Trains two Random Forest regressors on the residual (actual count minus the Hardy–Littlewood estimate):
   - **Model A** — additive-only features.
   - **Model B** — additive plus Erdős–Straus inversion aggregates.

Negative $\Delta(\mathrm{RMSE}, \mathrm{MAE})$ means Model B outperforms Model A, so the Erdős–Straus channel is carrying extra signal.
...
This uses the divisor identity with $A = k \cdot m - 1$ and $B = n \cdot m$:

```text
(A·b − B)(A·c − B) = B²
```

A 3-term solution exists iff $B^2$ has a factorization consistent with positive integers $b, c$. The gate is intentionally restricted (first denominator fixed to $n \cdot m$) to give a clean, reproducible obstruction score.

For each $(k, n)$ with $k \in \{4,5,6,7\}$ and $n \le N_{\max}$, it trains:
- **Classifier**: `solved_within_gate` — binary, whether a solution was found
...
These are **diagnostic tools, not proof engines**. A positive result (Erdős–Straus features improve held-out prediction) means the Erdős–Straus channel leaks extra information about Goldbach structure under the chosen featureization. It does **not** establish implication in either direction.
